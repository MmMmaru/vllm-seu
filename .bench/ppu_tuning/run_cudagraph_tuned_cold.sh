...
CACHE_ROOT="$TUNE_ROOT/cache/cudagraph_tuned_cold"
...
export ZSX_TUNED_CUDAGRAPHS=1
unset ZSX_FLASHINFER_AUTOTUNE
...
exec "$ROOT/.venv/bin/python" "$TUNE_ROOT/benchmark_public.py" \
  ...
  --num-samples 500 \
  --warmup-samples 2 \
  --output "$ROOT/result_cudagraph_tuned_cold_500.json"
